...
#async def send_random_value(message):
    # Логика вашей функции
   # await message.answer("Случайное значение: 42")  # Пример ответа
...
async def send_random_value(callback_query: CallbackQuery):
    # Подтверждаем получение callback (обязательно)
    await callback_query.answer("Успешно вызван callback!", show_alert=True)
